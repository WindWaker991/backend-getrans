...
export class AccountContactsController {
  ...
  @Get(':id')
  findBankAccountContacts(@Param('id') contactListId: string) {
    return this.accountContactsService.findBankAccountContacts(contactListId);
  }
  ...
  async update(
    @Param('id') id: string,
    @Body() updateAccountContactDto: UpdateAccountContactDto,
  ) {
